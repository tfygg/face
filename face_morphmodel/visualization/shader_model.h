...
private:
    ...
    int                 mNumVertices;
    int                 mNumTriangles;
    GLuint              mVAO, mVBO[3], mEBO;
public:
    MorphModelShader(int    numVertices,
                     int    numTriangles,
                     float *positionPtr,
                     float *normalPtr,
                     float *texCoordPotr,
                     int *  indicePtr)
        : snow::Shader()
        , mPositionPtr(positionPtr)
        , mNormalPtr(normalPtr)
        , mTexCoordPtr(texCoordPotr)
        , mIndicePtr(indicePtr)
        , mNumVertices(numVertices)
        , mNumTriangles(numTriangles)
        , mVAO(0), mVBO({0, 0, 0}), mEBO(0) {
        ...
        glBufferData(GL_ARRAY_BUFFER, sizeof(float) * 3 * mNumVertices, mPositionPtr, GL_DYNAMIC_DRAW);
